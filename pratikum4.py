...
print("=== PRATIKUM 4: PENINGKATAN KUALITAS CITRA ===")
print("Judul: Pipeline Enhancement Citra untuk Optimalisasi Visual Citra Underexposed dan Overexposed\n")

def load_sample_images():
    """Load sample with diffrent characteristics."""
    images = {}

    # load images in grayscale
    underexposed = cv2.imread("underexposed.jpg", cv2.IMREAD_GRAYSCALE)
    overexposed = cv2.imread("overexposed.jpg", cv2.IMREAD_GRAYSCALE)
    uneven = cv2.imread("uneven.jpg", cv2.IMREAD_GRAYSCALE)

    # Resize supaya ukurannya sama (biar rapi saat visualisasi)
    size = (256,256)

    underexposed = cv2.resize(underexposed, size)
    overexposed = cv2.resize(overexposed, size)
    uneven = cv2.resize(uneven, size)

    images['underexposed'] = underexposed
    images['overexposed'] = overexposed
    images['uneven_illumination'] = uneven
...
low_contrast_img = sample_images['uneven_illumination']
...
dark_image = sample_images['underexposed']
...
test_img = sample_images['uneven_illumination']
...
def calculate_enhancement_metrics(original, enhanced):
    ...
    orig_entropy = entropy(orig_hist + 1e-10)  # Add small value to avoid log(0)
    enh_entropy = entropy(enh_hist + 1e-10)
